...
const FindPwdBefore = () => {
  const randomKey = async (length: number): Promise<string> => {
    return Math.random().toString(16).slice(2, length);
  };

  const saveAuthKey = async (key: string, id: string): Promise<void> => {
  ...
  const openPopup = async (id: string, key: string): Promise<void> => {
    window.open(
