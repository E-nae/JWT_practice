...
const { storeKey, delKey, getFpKey } = require('../../util/keyManager');
...
router.post('/submit', async (req, res) => {
  ...
  try {
    ...
    const getProfile = async () => {
      try {
        const headers = header(process.env.HEADERS_METHOD, givenID, tuid1);
        const pf_data = {
          // 프로필
          LANG: 'KR',
          VIEW: 0,
        ...
        const position = posRes?.data?.data?.DATA.find(
          (pos) => pos.ID === profileRes?.data?.data?.DATA?.POSITION
        );
...
router.post('/logout', async (req, res) => {
  try {
    ...
    if (process.env.NODE_ENV === 'development') res.send({ ok: true });
  } catch (error) {
...
router.get('/pending', (req, res) => {
  res.send('waiting area for new users');
});
