...
const VerifyTK = ({ children }) => {
  const location = useLocation();
  const dispatch = useDispatch();
  const method = useSelector((state) => state.login.method); // 로그인 수단
  const { UID, setUID } = useUser();
  const { setIsAuthenticated } = useAuth();

  const excluded = LoginRoutes?.children?.map((obj) => obj.path).filter((item) => item !== '/' && item !== '/login');
  const isExcludedPage = excluded.some((path) => location.pathname?.startsWith(path));
  ...
  if (isError) {
    ...
    return dispatch(loginMethod({ method: 'unknown' }));
  }
