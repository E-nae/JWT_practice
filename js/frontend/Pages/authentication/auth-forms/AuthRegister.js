...
const AuthRegister = () => {
  ...
  const [idCheck, setIdCheck] = useState({ status: 'default', message: '' });
        ...
        onSubmit={async (values, { setErrors, setStatus, setSubmitting }) => {
          try {
            ...
            if (idCheck.status === 'pass') {
              ...
              values['EMAIL'] = { CONTENTS: values.EMAIL, IS_ACTIVE: 'MAIN', TY: 'EMAIL' };
              values['TEL'] = { CONTENTS: values.TEL, IS_ACTIVE: 'MAIN', TY: 'TEL' };
              values['ADDR'] = { CONTENTS: address, IS_ACTIVE: 'MAIN', TY: 'ADDRESS' };
              delete values.PASSWDCheck;
              console.log(values);
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={6}>
                <Stack spacing={1}>
                  <Button
                    onClick={() => {
                      sample6_execDaumPostcode(detailAddr.current.children[0]);
                    }}
                    size="large"
                    variant="outlined"
                    color="primary"
